import { IBaseRepository, IOptionsRepository, IPaginationRepository } from '@app/shared/interfaces';
import { Document, Model, UpdateQuery } from 'mongoose';

export abstract class BaseMongoRepository<TEntity, TCreate, TUpdate, TDocument extends Document>
    ...
    async getAll(options: IOptionsRepository<TEntity>): Promise<IPaginationRepository<TEntity>> {
    ...
    async update(id: string, item: TUpdate): Promise<TEntity> {
        const document = await this.model.findByIdAndUpdate(id, item as UpdateQuery<TDocument>, {
            new: true,
        }) as TDocument;
